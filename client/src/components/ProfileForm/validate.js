...
export const validate = (input) => {
  const { fullName, street, city, province, postalCode, country } = input;
  ...
  if (city.length <= 2 || city.length > 30) {
    errors.city_length = "The City must contain between 3 and 30 characters";
  }

  // if (!namecheck.test(province)) {
  //   errors.province = "Only number, letters and spaces are accepted";
  // } else
  ...
  //   errors.country = "Only number, letters and spaces are accepted";
  // } else
  ...
  //   errors.postalCode = "Only number, letters and spaces are accepted";
  // } else
